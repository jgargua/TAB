...
ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, alpha = Species)) +
  geom_point()
#Me sale un mensaje de error con size y con alpha porque no están recomendados. Hay que tener en cuenta que utilizar diferentes tamaños, pero la misma forma no es muy visual. Y en el caso de alpha se va aclarando el color del punto, que tampoco es muy visual
```
</div>
...
#Junta los dos tipos de transmisión en una única columna, en vez de tenerlos separados en dos.
```
...
ggplot(data = mtcars, mapping = aes(x = cyl_f, fill = gear)) +
  geom_bar(position = "dodge")
#El resultado es numérico, hay que transformarlo a un factor
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(. ~ cut)
#En vez de hacer las gráficas una encima de la otra, las coloca una al lado de la otra
```
...
Yo creo que es más fácil compararlas si comparten el eje X porque ves mśa facilmente las diferencias
<div style="background-color:#F0F0F0">
##### <i class="fas fa-comment-dots"></i> Answer:
...
#Viridis porque es una paleta pensada para que la gente que no puede diferenciar colores los diferencie
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) +
  labs(subtitle= "Diamonds", caption = "Weight variation and quality of diamond", tag= "Plot")
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .)  +
  theme_minimal() # theme that maximises the ink-ratio
#No tiene colores ni lineas que no son necesarios para leer los datos
```
...
#El archivo png se pixela si haces zoom, en cambio el svg no.
```
...
class(economics)
unemployment <- ggplot(data = economics, mapping = aes(x = date, y = unemploy)) +
  geom_line(color = "red") + labs(title = "Evolution of unemployment", x = "Year", y = "Unemployment") + theme_minimal()
ggsave(filename = "unemployment.png", plot = unemployment, width = 6, height = 4)
```
...
```{r readSorted}
#Para hacer esto utilizo el archivo sorted.depth.txt de mi población que es FIN
...
```{r answerP1.3}
ggplot(data = readDepth1, mapping = aes(x = position, y = depth, color = depth < 20)) + scale_colour_manual(values = c("indianred2", "cadetblue1")) + labs(title = "Read depth at the chromosome 2 region", x = "Position of chromosome 2", y = "Read depth", color = "Coverage < 20") + theme_minimal() +
  geom_point()
```
...
```{r ST}
st <- read.table("ST_FIN", sep='\t', header=FALSE)
...
st <- read.table("ST_FIN", sep='\t', header=FALSE)
colnames(st) <- c("ST", "ID", "type", "count")
ggplot(data = st, mapping = aes(x = type, y = count, fill= type)) +
  geom_col()
```
...
```{r answerP1.5}
indel <- read.table("indel_FIN.txt", sep='\t', header=FALSE)
head(indel)
...
ggplot(data = st, mapping = aes(x = type, y = count, fill=type)) + labs(title = "Substitutions", x = "Type of substitution", y = "Total number") + theme_minimal() +
  geom_col()
```
...
```{r pi, eval = F}
pi <- read.table("out.windowed.pi", sep='\t', header=TRUE)
...
```{r answerP1.6}
r <- ggplot(data = pi, mapping = aes(x = BIN_START, y = PI)) + geom_line(color="mediumvioletred") +
  ggtitle("Nucleotide diversity") +
  theme_bw() +
  labs(x= "Position", y= "Nucleotide diversity")
r

ggsave(filename = "Nucleotide_diversity_FIN_plot.png", plot = r, width = 6, height = 4)
```
...
```{r answerP1.7}
snpden <- read.table("out.snpden", sep='\t', header=TRUE)

head(snpden)
snpdensity <- ggplot(data = snpden, mapping = aes(x = BIN_START, y = VARIANTS.KB)) + geom_line(color="darkslategrey") +
  ggtitle("SNP density") +
  theme_bw() +
  labs(x= "Position", y= "SNP density")
snpdensity

snpcount <- ggplot(data = snpden, mapping = aes(x = BIN_START, y = SNP_COUNT)) + geom_line(color="brown1") +
  ggtitle("SNP count") +
  theme_bw() +
  labs(x= "Position", y= "SNP count")
snpcount

ggsave(filename = "SNP_density_FIN_plot.png", plot = r, width = 6, height = 4)
ggsave(filename = "SNP_count_FIN_plot.png", plot = r, width = 6, height = 4)
```
...
```{r answerP1.8}
VEP <- read.table("VEP.txt", sep='\t', header=TRUE)
head (VEP)

ggplot(data = VEP, mapping = aes(x = BIOTYPE)) + labs(title = "VEP results", x = "Genomic element", y = "Number of SNPs") + theme_minimal() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
