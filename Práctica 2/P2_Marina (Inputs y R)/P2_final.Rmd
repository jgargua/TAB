...
ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, alpha = Species)) +
  geom_point()
#No está recomendado usar tamaño o alpha (transparencia) porque se ven menos claros los resultados. 
```
</div>
...
#Ahora en vez de darnos el número total, lo relativiza a 1 y nos da las proporciones para que podamos comparar mejor. 
```
...
#Primero tenemos que transformar gear en un factor. 
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(. ~ cut)
#Ahora en vez de hacer el corte en horizontal, lo hace en vertical. 
...
<div style="background-color:#F0F0F0">
##### <i class="fas fa-comment-dots"></i> Answer:
Es más fácil si comparten el eje x, porque vemos si alguna está más desplazada. 
</div>
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) +
  scale_fill_manual(values = c("sienna1", "plum2", "lightseagreen", "orangered", "red4"))
```
...
# Option A:
p + scale_fill_manual(values = c("sienna1", "plum2", "lightseagreen", "orangered", "red4"), name = "Quality") +
  scale_x_continuous(name = "Weight of the diamond") + 
  ggtitle("Diamond weight variation")

# Option B:
# p + scale_fill_manual(values = c("sienna1", "plum2", "lightseagreen", "orangered", "red4")) +
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) +
  scale_fill_manual(values = c("sienna1", "plum2", "lightseagreen", "orangered", "red4"), name = "Quality") +
  scale_x_continuous(name = "Weight of the diamond") + 
  ggtitle("Diamond weight variation") +
  theme_bw()
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  facet_grid(cut ~ .) +
  scale_fill_viridis_d()
#El más recomendado sería el viridis porque las personas con defectos de visión como el daltonismo tienen problemas para distinguir verde y rojo, y en esta paleta no tenemos rojo, por lo que no puede dar luagr a confusiones tan fácilmente. 
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) +
  labs(subtitle = " Gráfica ", caption = " Caption ", tag = " Diamantes ")
#El tag engloba todo el gráfico y se coloca en la izquierda arriba. El subtitle está debajo dle título del gráfico para aclarar. La caption se encuentra debajo del gráfico para dar aclaraciones o explicaciones. 
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  facet_grid(cut ~ .) +
  theme_minimal()
#Estos dos serían los más data-ink ratio. 
```
...
#Vemos que utilizando el svg no se pixela cuando haces zoom in y, en cambio, el de png sí que se pixela. 
```
...
p <- ggplot(data = economics, mapping = aes(x = date, y = unemploy)) +
  ...
  labs(title = "Evolution of unemployement", x = "Year", y = "Unemployement") +
  theme_bw()
ggsave(filename = "plotorange.png", plot = p, width = 6, height = 4)
p
...
```{r readSorted}
readDepth <- read.table("sorted.depth.txt.gz", sep='\t', header=FALSE)
...
```{r ST}
st <- read.table("ST.txt", sep='\t', header=FALSE)
...
```{r answerP1.5}
indels <- read.table("indel.txt", sep='\t', header=FALSE)
colnames(indels) <- c("IDD", "ID", "length", "count")
head(indels)
ggplot(data = indels, mapping = aes(x = length, y = count)) +
  geom_col(fill = "orange") +
 labs(title = "Indel length distribution", x = "Indel length", y = "Total number") +
  theme_bw() 
```
...
ggplot(data = st, mapping = aes(x = type, y = count)) +
  geom_col(fill = "orange") +
 labs(title = "Substitutions", x = "Types of substitutions", y = "Total number") +
  theme_bw() 
...
ggplot(data = pi, mapping = aes(x = BIN_START, y = PI)) +
  geom_line(color = "orange") +
  labs(title = "Nucleotide diversity along the chromosome 2 region", x = "Genomic position", y = "Nucleotide diversity") +
  theme_bw()
```
...
ggplot(data = snpden, mapping = aes(x = BIN_START, y = SNP_COUNT)) +
  geom_line(color = "orange") +
    labs(title = "SNP count along the chromosome 2 region", x = "Genomic position", y = "SNP count") +
  theme_bw()
ggplot(data = snpden, mapping = aes(x = BIN_START, y = VARIANTS.KB)) +
  geom_line(color = "orange") +
    labs(title = "Variants/KB along the chromosome 2 region", x = "Genomic position", y = "Variants/KB") +
  theme_bw()

#Los plots son iguales porque las variantes/kb son los SNPs que hay en cada kb y te cuenta SNPs por cada kb. 
```
...
```{r answerP1.8}
vep <- read.table("VEPfile.txt", sep='\t', header=TRUE)
ggplot(data = vep, mapping = aes(x = BIOTYPE)) +
  geom_bar(fill = "orange") +
 labs(title = "VEP results", x = "Genomic element", y = "Number of SNPs") +
