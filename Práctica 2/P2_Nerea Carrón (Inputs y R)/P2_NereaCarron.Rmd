...
#Guardamos plot
ggsave(filename = "plot_pi_IBS.png", plot = plot_pi, width = 6, height = 4)
```
...
## CONSEQUENCE
vep <- read.table(file = "Data/IBS.vep.txt", sep = '\t', header = FALSE)
head(vep)

plot_cons_ibs <- ggplot(data = vep, mapping = aes(x = V4)) +
  geom_bar(fill = "orange") + 
  labs(title="VEP Results. Functional consequence of SNPs", x = "Consequences", y= "Number of SNPs") + 
  theme_bw()
plot_cons_ibs <- plot_cons_ibs + theme(axis.text.x = element_text(size = 5, angle = 60, hjust = 1))
  
ggsave(filename = "plot_IBS_vep_cons.jpeg", plot = plot_cons_ibs, width = 6, height = 4)


## BIOTYPE
plot_IBSvep_biot <- ggplot (data = vep, mapping = aes(x = V10)) +
  geom_bar(fill = "#FF9999") +
  labs(title="VEP Results. Type of the genomic elements", x = "Genomic element", y= "Number of SNPs") + theme_bw()

plot_IBSvep_biot <- plot_IBSvep_biot + theme(axis.text.x = element_text(angle = 60, hjust = 1))


ggsave(filename = "plot_IBSvep_biot.jpeg", plot = last_plot(), width = 6, height = 4)
