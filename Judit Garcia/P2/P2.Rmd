...
ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, alpha = Species)) +
  geom_point()
```
Yes, there's a warning message saying using size and alpha as a discrete variable is nor advised. They're not a clear way of checking data diferences, as diferent sizes of circles or diferent transparency can't be clearly diferentiated.
</div>
...
```
It's adjusting the plot so it will fill the whole window. It transforms the count data into a 0->1 range.
</div>
...
class(mtcars$gear)
#result is numeric, so we have to transform it to a factor
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  facet_grid(. ~ cut)
```
Instead of stacking them one above the other, they are one beside the other.
</div>
...
<div style="background-color:#F0F0F0">
##### <i class="fas fa-comment-dots"></i> Answer:
It's easier to see the diferences if they share the x axis, because you can clearly see if one of them diferences from the others. So, the best one would be the (cut ~ .) one.
</div>
...
```
The best one would be the viridis one, since it doesn't have colors that colorblind people struggle to diferentiate.
</div>
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + labs(subtitle = "Diamonds", caption ="Diamond weight variation and quality", tag ="Plot")
```
The subtitle tag adds a subtitle under the title. Caption adds a comment under the plot. Tag adds a title for the plot and caption.
</div>
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + theme_minimal()
```
The minimal theme maximises the data-ink, it doesn't have added colors or lines that are not needed to read the data.
</div>
...
```
The png gets pixelated when you zoom in, the svg one doesn't.
</div>
...
class(economics)
lineplot <- ggplot(data = economics, mapping = aes(x = date, y = unemploy)) +
  geom_line(color = "red") + labs(title = "Evolution of unemployment", x = "Year", y = "Unemployment") + theme_minimal()
ggsave(filename = "lineplot.png", plot = lineplot, width = 6, height = 4)
```
...
```{r readSorted}
readDepth <- read.table("https://bioinformatica.uab.cat/base/documents/sgbcursos/documents/depth2019_11_6D10_16_25.txt", sep='\t', header=FALSE)
...
```{r answerP1.1}
readDepth <- read.table("/home/samba/homes/1421491/SERVER/HOME/TAB/P1/NA20502.TSI.chr2_134800000-137500000_sorted.depth.txt.gz", sep='\t', header=FALSE)
colnames(readDepth) <- c("chr", "position", "depth")
readDepth1 <- readDepth[1:10000,]
ggplot(data = readDepth1, mapping = aes(x = position, y = depth)) +
  geom_point()
```
...
```{r answerP1.2}
ggplot(data = readDepth1, mapping = aes(x = position, y = depth, color = depth < 20)) +
  geom_point()
```
...
```{r answerP1.3}
ggplot(data = readDepth1, mapping = aes(x = position, y = depth, color = depth < 20)) + scale_colour_manual(values = c("black", "red")) + labs(title = "Read depth along the chromosome 2 region", x = "Position of chromosome 2", y = "Read depth", color = "Coverage < 20") + theme_minimal() +
  geom_point()
...
```{r answerP1.4}
st <- read.table("/home/samba/homes/1421491/SERVER/HOME/TAB/P1/ST.txt", sep='\t', header=FALSE)
colnames(st) <- c("ST", "ID", "type", "count")
ggplot(data = st, mapping = aes(x = type, y = count)) +
  geom_col()
```
...
```{r answerP1.5}
indel <- read.table("/home/samba/homes/1421491/SERVER/HOME/TAB/P1/indel.txt", sep='\t', header=FALSE)
head(indel)
colnames(indel) <- c("IDD", "ID", "length", "count")
ggplot(data = indel, mapping = aes(x = length, y = count)) +
  geom_col()
```
...
```{r answerP1.5A}
ggplot(data = indel, mapping = aes(x = length, y = count)) + labs(title = "Indel length distribution", x = "Indel length", y = "Total number") + theme_minimal() +
  geom_col(fill = "red")
ggplot(data = st, mapping = aes(x = type, y = count)) + labs(title = "Substitutions", x = "Type of substitution", y = "Total number") + theme_minimal() +
  geom_col(fill = "red")
```
...
```{r answerP1.6}
ggplot(data = pi, mapping = aes(x = BIN_START, y = PI)) +
  geom_line()
```
...
```{r answerP1.7}
snpden <- read.table("https://bioinformatica.uab.cat/base/documents/sgbcursos/documents/snpDensity2019_11_5D12_46_19.txt", sep='\t', header=TRUE)
head(snpden)
ggplot(data = snpden, mapping = aes(x = BIN_START, y = VARIANTS.KB)) +
  geom_line()
ggplot(data = snpden, mapping = aes(x = BIN_START, y = SNP_COUNT)) +
  geom_line()
```
...
```{r answerP1.7A}
ggplot(data = pi, mapping = aes(x = BIN_START, y = PI)) + labs(title = "Nucleotide diversity along the chromosome 2 region", x = "Genomic position", y = "Nucleotide diversity") + theme_minimal() +
  geom_line(color = "red")
```
...
```{r answerP1.8}
vepfile <- read.table("/media/1421491/JUDIT/TAB/P1/VEP/SY7odsousNEAzYUC.txt", sep='\t', header=TRUE)
head(vepfile)
ggplot(data = vepfile, mapping = aes(x = BIOTYPE)) + labs(title = "VEP results", x = "Genomic element", y = "Number of SNPs") + theme_minimal() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  geom_bar(fill = "red")
```
