...
getwd()
setwd("C:/Users/Judit/Desktop/TAB FINAL/P4/Project4")
...
# ggplot2 plot
ggplot(data = dataGroups, mapping = aes(x = Stage, fill = Group )) + labs(y = "Number of patients") +
  geom_bar()
```
...
3 columns: gene_id, bp_length and symbol. We have the gene id, the length of the gene in bp and the name of the gene.
</div>
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
total number of individuals: 87
filtered individuals: 87
individuals by stage: 26 early stage, 61 in the late stage
number of genes: 58037 genes
</div>
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
- We can't observe any improvement. If we compare the raw data with RPKM we can see that the normalization hasn't helped, it has made it worse. The red line is even further away from 0 in the RPKM plot.
- Comparing the raw data with the TMM method we can see an improvement, the red line is now more accurately over the the y = 0 line.
- In this case, the TMM method would be better to normalize the data.
</div>
...
ggplot(data = resDF, mapping = aes(x = log2FoldChange, y = -log10(padj))) + theme_minimal() + scale_fill_viridis_d() + labs(title = "Volcano plot", x = "Effect size: log2(fold-change)", y = "-log10(adjusted p-value)") +
  geom_point(aes (colour = filter)) + geom_label_repel(data=resDF[abs(resDF$log2FoldChange) > log2(10) & resDF$padj < 0.001 ,], aes(label = as.factor(gene)), alpha = 0.7, size = 5, force = 1.3)  
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
There's 32 genes in the new dataset. 
</div>
...
```{r data-frame-fig2}
# ggplot2 plot
ggplot(data = GOresult, mapping = aes(x = Term, y = Count)) + labs(title = "Gene Ontology enrichment analysis", x = "GO term") + theme_minimal() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  geom_col(fill = "red")
```
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
Most of the genes have different terms, there's only a few that terms that have 5 or more genes.
Multicellular organismal movement is the one that has the most genes (more than 15).
</div>
