...
# Alpha es transparencia. No están recomendados size o alpha para estos tipos de datos.
```
</div>
...
#Ha hecho las proporciones de cada una para ponerlo sobre 1.
```
...
class(mtcars$gear_f) #[1] "factor"
ggplot(data = mtcars, mapping = aes(x = cyl_f, fill = gear_f)) +
  geom_bar(position = "dodge")
#Si no ponemos gear como factor no te saber dividirlo en categorias porque para el es numerico.
```
...
#Mira cuantas hay del 0 al 0,3 del 0,3 al 0,6 y así.
```
...
#Con fill=cut pintamos el histograma en función del cut.
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(. ~ cut)
#En función de donde está el cut corta por un lado u otro. si el cut está a la derecha corta por el x y si está a la izquierda por el y.
```
...
<div style="background-color:#F0F0F0">
##### <i class="fas fa-comment-dots"></i> Answer: Si queremos comparar la distribución de las funciones es más facil la primera (cuando el eje de la x es igual para todos)
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + scale_fill_hue()
  # scale_fill_...  # try other scale_fill
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + scale_fill_viridis_d()
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + scale_fill_brewer()
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + scale_fill_grey()
#Va cambiando el color de los cuadraditos y las barras. 

#Utilizaria la pleta scale_fill_viridis_d() para que las personas daltonicas la vean bien.
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + labs(subtitle = "Title graphic", caption = "Here caption", tag = "Graphic")

#Estos argumentos de label permite escribir en diferentes partes del gráfico. tag es un titulo grande, subtitle es más pequeño y caption esta abajo a la derecha.
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) +
  theme_bw() # theme that maximises the ink-ratio
# theme_minimal()
```
...
```{r dev, eval = FALSE}
png(filename = "plot1.png", width = 600, height = 400, res = 150) # In pixels by default
p
dev.off()

#Aunque ponga null lo esta haciendo
```
...
```{r answer 5.1}
ggsave(filename = "plot_jpeg.jpeg", plot = p, width = 600, height = 400, units = "mm") # In inches by default
ggsave(filename = "plot_svg.svg", plot = p, width = 6, height = 4) # In inches by default

#Uno se pixela (jpeg) y el otro no (svg).
```
...
plot_economics <- ggplot(data = economics, aes(x = date, y = unemploy)) +
  geom_line(colour = "blue") +
  labs(title = "Evolution of unemployment", x= "Years", y="unemployment")+
  theme_bw()
ggsave(filename = "plot_economics.png", plot = plot_economics, width = 6, height = 4)
...
```{r readSorted}
readDepth <- read.table("Data/sorted.depth.txt.gz", sep = '\t', header = FALSE)
```
...
```{r answerP1.3}
plot_depth <- ggplot(data = readDepth, mapping = aes(x = position, y = depth, color = depth < 20)) +
  geom_point() +
  scale_colour_manual(values = c("black", "red")) + 
  labs(title = "Read depth along the chromosome 2 region", x= "Position of chromosome 2", y="Read depth", color = "Coverage < 20") +
    theme_bw()

## Guardamos el gráfico para subirlo a github
ggsave(filename = "plot_depth.jpeg", plot = plot_depth, width = 6, height = 4)
```
...
```{r ST}
st <- read.table("Data/IBS_ST.txt", sep='\t', header=FALSE)
...
```{r answerP1.4}
plot_snp <- ggplot(data = st, mapping = aes(x = type, y = count)) + geom_col(fill='orange') +
  labs(title = "Substitutions", x= "Type of substitution", y="Total   number") +
    theme_bw()

## Guardamos el gráfico 
ggsave(filename = "plot_snp_IBS.png", plot = plot_snp, width = 6, height = 4)
...
```{r answerP1.5}
#Guardamos los datos en un objeto y le ponemos nombre a las columnas.
indels <- read.table("Data/IBS_Indel.txt", sep='\t', header=FALSE)
colnames(indels) <- c("IDD", "ID", "length", "count")
head(indels)

#Hacemos un plot con las longitudes y lo guardamos
plot_indels <- ggplot(data = indels, mapping = aes(x = length, y = count)) +  geom_col(fill = "green") +  labs(title = "Indel length distribution", x="Indel length", y = "Total number") +  theme_bw()

ggsave(filename = "plot_indels_IBS.png", plot = plot_indels, width = 6, height = 4)
```
...
```{r answerP1.5A}
#Pintamos el gráfico de dos colores, en función de si es una inserción o una delección.
plot_indels_difcol <- ggplot(data = indels, mapping = aes(x=length, y = count, fill = length < 0)) + 
  geom_col() +
  scale_fill_manual(values = c("blue", "green")) +
  labs(title = "Indel length distribution", x="Indel length", y = "Total number", color ="Length < 0") + 
  theme_bw()

#Guardamos el gráfico
ggsave(filename = "plot_indels_IBS_2colores.png", plot = plot_indels_difcol, width = 6, height = 4)
```
...
```{r pi, eval = F}
pi <- read.table("Data/out.windowed.pi", sep = '\t', header=TRUE)
head(pi)
```
...
```{r answerP1.6}
plot_pi <- ggplot(data = pi, aes(y = PI, x = BIN_START)) +
  geom_line(colour = "blue") +
  labs(title = "Nucleotid diversity along the chromosome 2 region", x= "Position", y="Nucleotide diverity")+
  theme_bw()
plot_pi

#Guardamos plot
ggsave(filename = "plot_pi_Eur.png", plot = plot_pi, width = 6, height = 4)
```
...
```{r answerP1.7}
snp <- read.table(file = "Data/out.snpden", sep='\t', header=TRUE)
head(snp)

### VARIANTS.KB
plot_snp <- ggplot(data = snp, mapping = aes(x = BIN_START, y = VARIANTS.KB)) + 
  geom_line(colour = "purple") +
  labs(title = "SNP density along the chromosome 2 region", x= "Position", y="SNP density")+
  theme_bw()
plot_snp

#Guardamos el plot
ggsave(filename = "plot_snp_variants.kb.png", plot = last_plot(), width = 6, height = 4)


### SNP_COUNT
plot_snp_count <- ggplot(data = snp, mapping = aes(x = BIN_START, y = SNP_COUNT)) + 
  geom_line(colour = "red") +
  labs(title = "SNP density along the chromosome 2 region", x= "Position", y="SNP Count")+
  theme_bw()
plot_snp_count

#Guardamos el plot
ggsave(filename = "plot_snp_count.png", plot = last_plot(), width = 6, height = 4)
...
```{r answerP1.8}
### EUR data -> 4 individuos

## CONSEQUENCE
vep <- read.table(file = "Data/EUR.vep.txt", sep = '\t', header = TRUE)
head(vep)

plot_cons_eur <- ggplot(data = vep, mapping = aes(x = Consequence)) +
  geom_bar(fill = "orange") + 
  labs(title="VEP Results. Functional consequence of SNPs", x = "Consequences", y= "Number of SNPs") + 
  theme_bw()
plot_cons_eur <- plot_cons_eur + theme(axis.text.x = element_text(size = 5, angle = 60, hjust = 1))
  
ggsave(filename = "plot_EUR_vep_cons.jpeg", plot = plot_cons_eur, width = 6, height = 4)


## BIOTYPE
plot_EURvep_biot <- ggplot (data = vep, mapping = aes(x = BIOTYPE)) +
  geom_bar(fill = "#FF9999") +
  labs(title="VEP Results. Type of the genomic elements", x = "Genomic element", y= "Number of SNPs") + theme_bw()

plot_EURvep_biot <- plot_EURvep_biot + theme(axis.text.x = element_text(angle = 60, hjust = 1))


ggsave(filename = "plot_EURvep_biot.jpeg", plot = last_plot(), width = 6, height = 4)
